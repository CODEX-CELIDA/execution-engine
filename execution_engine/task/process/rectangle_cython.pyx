...
def find_overlapping_windows(windows: list[Interval], intervals: list[Interval]) -> list[Interval]:
    """
    ...
    while w_idx < len(window_events) and i_idx < len(interval_events):
        ...
        if window_events[w_idx][0] <= interval_events[i_idx][0]:
            ...
            if opening:
                ...
                if active_intervals > 0:
                    intersecting_windows.append(
                        Interval(event_time, window_events[w_idx][0] - 1, window_events[w_idx][2]))
            ...
            if active_intervals > 0 and active_window:
                intersecting_windows.append(
                    Interval(window_events[w_idx - 1][0], window_events[w_idx][0] - 1, window_events[w_idx][2]))
                w_idx += 1  # we can move on to the next window
                active_window = False
