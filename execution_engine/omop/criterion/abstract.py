...
class Criterion(AbstractCriterion):
    ...
    def sql_select_data(self) -> None:
        ...
        select(
            self._table.c.person_id,
            bindparam("run_id", bindparam("run_id")).label("recommendation_run_id"),
            bindparam("cohort_category", self.category.name).label("cohort_category"),
            bindparam("criterion_name", self.name).label("criterion_name"),
            bindparam("domain_id", self._OMOP_DOMAIN).label("domain_id"),
        )
    ...
    def sql_insert_into_table(
        sql: Select, table: TableClause, temporary: bool = True
    ) -> SelectInto:
        """
        Insert the result of the query into the result table.
        """
        if not isinstance(sql, Select):
            raise ValueError("sql must be a Select")

        query = select_into(sql, table, temporary=temporary)
